                           ...
                           struct dp_packet **packets, int *c)
{
    ...
    if (OVS_UNLIKELY(!is_vhost_running(virtio_dev))) {
	    //printf("vvdn debug : vhost is not running dev : %p\n",virtio_dev);
        return EAGAIN;
    }

    if (rxq_->queue_id >= vhost_dev->real_n_rxq) {
	printf("vvdn debug : EOPNOTSUPP\n");
        return EOPNOTSUPP;
    }
    ...
    printf("vvdn debug : func : %s received packets : %d bytes\n",__func__,nb_rx);
